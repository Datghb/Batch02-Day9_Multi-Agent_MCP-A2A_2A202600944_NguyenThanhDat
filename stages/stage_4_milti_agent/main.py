...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data, privacy, GDPR, CCPA, consent, data breach'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    needs_privacy = bool(parsed.get("needs_privacy", False)) or any(
        kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "consent", "breach"]
    )
    print(
        "  [Node: check_routing] "
...
async def call_privacy_specialist(state: LegalState) -> dict:
    """Privacy specialist sub-agent for GDPR and data protection analysis."""
    print("\n  [Node: call_privacy_specialist] Privacy specialist agent starting...")
    llm = get_llm()
    messages = [
        SystemMessage(
            content=(
                "You are a specialist in GDPR, CCPA, privacy law, data protection, "
                "breach notification, consent, and data subject rights. Analyse only "
                "the privacy and data-protection issues. Keep your response under 200 words."
            )
        ),
        HumanMessage(
            content=(
...
async def main():
    ...
    print("  ./start_all.sh && uv run python test_client.py")
    print("=" * 70)
